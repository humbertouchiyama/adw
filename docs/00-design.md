...
| **blocked** | a finding cleared the escalation bar, the diff touches a gate file, the **review**-cycle cap was reached and `/pr-ready` §3's closure check still found something to apply (§4.8 condition 1), `/pr-ready` §3's open-findings check returned `open` or `unclear` for an earlier `Blocking`, `/pr-ready` §5 found a `Blocking` still open under a review's `🔧 To fix`, or the 16-boundary cap fired (condition 6 — gate-green but unconverged) | PR open, labelled, body per §6.1 S4. A blocked sub-PR halts its chain |
